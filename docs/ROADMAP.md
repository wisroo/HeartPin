...
> 작성일: 2026-06-05 · 기준 코드: 프론트엔드 프로토타입 완성 (데이터 메모리 상태, [wisroo/HeartPin](https://github.com/wisroo/HeartPin))
>
...
```
앱 코드 ──→ StorageAdapter (인증 · 사진 저장 · 기록 CRUD · 정리함 CRUD)
...
| 1-2. StorageAdapter + 데이터 레이어 교체 | 현재 메모리(HP_DATA) → 어댑터 경유 CRUD. 인터페이스 규율 준수 | ~1.5–2일 |
| 1-3. 업로드 파이프라인 | 클라 압축(2000px/400px WebP) + EXIF + **SHA-256 해시** + **owner 토글** + 중복 감지 | ~1일 |
| 1-4. 보관함(외장하드) 연결 | FS API: 폴더 선택→핸들 영속화→스캔/해시 매칭→새 사진 복사·흐릿 삭제(`discard_pending` 큐)·`record-backup.json` | ~1.5일 |
...
| 2-3. 모바일 업로드 다듬기 | Wi-Fi 전용 옵션·진행률 UI·**iOS HEIC/EXIF 실기기 검증** | ~1일 |
...
```
Phase 1  MVP (외장하드+공유 기록)   ████████░░  ~5–6일   ← 지금 여기
...
| iOS 웹 사진 선택기 | HEIC→JPEG 변환·메타데이터 처리가 OS 버전마다 다름 | Phase 2에서 실기기 검증. 위치 빠지면 "위치 없음" 플로우로 수용 |
...
- [ ] 압축 파라미터 확정 — 실사진으로 2000px/q80 화질·용량 검증
