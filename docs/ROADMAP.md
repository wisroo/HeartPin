...
| 여행 상세 지도 의미적 줌아웃 | **2026-07-17 사용자 검증에서 Web·Mobile 모두 줌아웃 후 전체 여행 지도로 복귀하지 않는 현상 재현.** 구현 커밋 `93dd4fd`의 자동 테스트와 자동 브라우저 경로는 통과했지만 실제 사용자 입력 경로를 충분히 대표하지 못하므로 미해결 버그로 유지 | 명시적 뒤로가기/`전체 여정`을 임시 사용. 실제 입력 이벤트와 Leaflet 줌 값을 계측해 재현 테스트를 먼저 추가한 뒤 수정 |
