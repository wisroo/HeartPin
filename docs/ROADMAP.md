...
> 작성일: 2026-06-05 · **갱신: 2026-07-14 (모바일 셸 선행 구현 · 원본 수령 플로우 보강)** · 기준 코드: Web/Mobile dual shell + Capacitor/Supabase 업로드 spike
>
...
| **상대방 원본 수령** — 요청한 원본을 내 폰/PC에 저장하고 사본 상태를 추적 | 모든 기기에 원본을 자동 복제하는 범용 동기화 |
...
| **D13** | 상대방 원본 저장 = **요청 기반 수령 + 사본 상태 추적** | 앱에서 보는 것은 압축본이면 충분하지만, 각자 폰에 원본을 갖는 니즈가 크다. 원본은 요청 시에만 임시 우체통을 거쳐 수령자 기기에 저장하고, 저장 성공 여부를 `photo_copies`에 남긴다 | 모든 원본 자동 양방향 동기화(용량·권한·프라이버시 부담 큼) |
...
| 폰 (둘 다) | 출발지(소스) + 수령지 | 업로드 받기 · 상대방 원본 요청/수령 · 하드 안착 후 **"폰에서 지워도 안전 ✓"** 표시 (웹은 카메라롤 저장/삭제가 제한됨) |
...
-- Phase 3에서 추가
photo_copies(photo_id, location, status)        -- 위치별 사본 추적 (예: bara_phone, nyong_phone, vault_drive, temp_cloud)
transfer_queue(id, photo_id, source, dest, tmp_url, status, expires_at)  -- 우체통: 요청→임시 업로드→수령→삭제
```
...
| 작업 | 내용 | 공수 |
|---|---|---|
| 3-1. 원본 요청 큐 | 압축본을 보다가 "원본 받기" 요청 → `transfer_queue`에 source/dest/status 생성. 원본 소유자 폰·PC·하드 중 가능한 출발지를 표시 | ~0.5일 |
| 3-2. 우체통(원본 릴레이) | 소유자 기기 또는 하드 세션에서 원본을 임시 클라우드 prefix에 업로드 → 수령 완료 후 삭제. 배치 용량 제한(또는 R2 버퍼) | ~1일 |
| 3-3. 상대방 폰/PC에 원본 저장 | 수령자가 다운로드하고 `photo_copies`에 `bara_phone`/`nyong_phone`/`personal_pc` 상태를 `saved`로 기록. 웹은 파일 다운로드, Capacitor는 카메라롤 저장으로 확장 | ~1일 |
| 3-4. 보관 현황 대시보드 | "우리 기록 1,240장 — 바라 폰 800 · 뇽이 폰 650 · 하드 1,100 · ⚠️ 원본 없는 사진 3" (`photo_copies`) | ~1일 |
| 3-5. "폰에서 지워도 안전" | 하드 또는 상대방 기기에 사본이 확인된 사진 목록 표시 — 웹은 표시까지, 네이티브 앱은 삭제 실행까지 | ~0.5일 |
...
Phase 1B MVP (Supabase 전환)        다음 핵심 작업 — 실제 기록 영속화 미완료
Phase 2  모바일 웹/PWA 보강          일부 선행 완료 — 모바일 디자인 셸/탭 UI 구현, PWA 보강 남음
Phase 3  전송·공동관리              미착수 — 원본 요청/상대방 폰 저장/보관 현황
...
| Supabase MVP 영속화 | Phase 1B 초입. schema/docs/spike는 있으나 실제 여행/스팟/모먼트 CRUD 영속화는 남음 |
| 원본 전송/상대방 폰 저장 | Phase 3로 보강. 아직 구현 전 |

따라서 현재 위치는 **Phase 1A 후반 + Phase 2 일부 선행 완료 + Phase 1B 진입 직전/초입**으로 본다. 다음으로 해야 할 일은 Phase 1B의 Supabase MVP 영속화다.
...
8. **상대방 원본을 내 폰에 저장** → 하드 보관과 별개의 핵심 플로우. 원본 요청→우체통→수령자 기기 저장→`photo_copies` 갱신→임시 원본 삭제로 처리한다. 웹은 다운로드 중심, Capacitor 앱은 카메라롤 저장까지 담당한다.
