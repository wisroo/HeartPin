...
- `storage.objects` 정책
- Supabase mobile upload spike를 보존하기 위한 legacy `test_uploads` 테이블
...
원본 사진은 Supabase에 영구 보관하지 않는다. Supabase Storage는 표시용 display 압축본과 thumb 썸네일만 맡는다. `test-originals/`는 Galaxy/iPhone 실기기 업로드 검증을 위해 남겨 둔 legacy 임시 prefix다.
...
3. `npm run dev` 또는 `npm run cap:sync` 후 앱을 연다.
4. Supabase 공유 계정으로 로그인한다.
5. 사진을 업로드한다.
6. Storage에서 `photos/display/<content_hash>.webp`, `photos/thumb/<content_hash>.webp` object가 생성됐는지 확인한다.
7. Table Editor에서 `inbox_items.content_hash`, `inbox_items.display_path`, `inbox_items.thumb_path`, `inbox_items.owner`, `inbox_items.lat/lng`, `inbox_items.taken_at`을 확인한다.

Capacitor native picker의 원본 URI/GPS 보존만 따로 검증해야 할 때는 legacy `test-originals/` + `test_uploads` 경로를 사용할 수 있다. 이 경로의 원본은 임시 검증물이며 Storage API 또는 Supabase dashboard에서 삭제한다.
...
Android 브라우저 사진 선택기가 위치 EXIF를 제거하는 문제가 확인됐으므로, 모바일 업로드는 Capacitor dev app에서 `inbox_items`까지 이어지는 경로로 검증한다.

1. Galaxy/iPhone Capacitor 앱에서 `inbox_items.lat/lng` 보존 확인
2. 업로드 중복 감지와 사용자-facing duplicate 결과 정리
3. `record.json` 마이그레이션 스크립트 작성
4. 외장하드 보관함 연결: FS Access API scan/copy/discard queue
5. legacy `test-originals` cleanup 도구 추가
