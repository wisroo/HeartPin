...
12. **Supabase 로그인 페이지.** 디자인 HTML엔 없는 추가 화면. 인증 모델 = `docs/PHASE1_SUPABASE.md`대로 **공유 계정 1개 + email/password** + 앱 내부 owner(바라/뇽이) 토글. 기본 스코프(가정, 변경 가능): **이번 브랜치는 로그인 UI만** + 얇은 auth seam(임시 세션), 실제 Supabase sign-in/SDK/adapter는 Phase 1 브랜치에서. (Supabase 백엔드가 아직 stub: `supabaseAdapter`는 throw, `@supabase/supabase-js` 미설치, env 비어있음)
...
```
src/mobile/
  MobileApp.jsx          (수정) 진입 게이트 라우팅: 로그인 → 첫실행 pick-side → 스플래시 → 탭셸
  onboarding/
...
**진입 게이트 순서:**
```
앱 열기 → [세션 없으면 LoginScreen] → [첫 실행/myChar 없으면 PickSide(1회)] → LaunchSplash(자동) → 여정
```
세션 있는 재방문자는 로그인·pick-side 건너뛰고 바로 스플래시. `MobileApp`이 `useAuth`(세션)와 `settings.myChar`(첫실행 여부)로 게이트를 판정.

- **진입 = 스플래시**(`LaunchSplash`). 게이트 통과 후. 버튼 없음. ~1.5s 후 `app.openMain()`, 탭하면 즉시. (저감 모션 사용자 위해 모션 최소화 고려)
