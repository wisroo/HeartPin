...
11. **첫 실행 1회 "pick your side" 온보딩.** 첫 진입(또는 myChar 미설정) 시 바라/뇽이 중 "나"를 고르는 일회성 페이지. 이후 실행에선 스킵. Couple/Settings에서 나중에 변경 가능. (owner pick을 Settings에만 두려던 안을 대체)
12. **Supabase 로그인 페이지 (재제작 + 디자인 톤).** 인증 모델 = `docs/PHASE1_SUPABASE.md`대로 **공유 계정 1개 + email/password** + 앱 내부 owner(바라/뇽이) 토글. 이 브랜치는 **main 기반**(spike 미통합)이라 로그인 UI를 디자인 톤(코랄·로고·바라♥뇽이)으로 새로 만들고 얇은 `useAuth` seam에 연결한다. 실제 동작 Supabase 로그인은 이미 **spike 브랜치**에 존재(`api.signInToSupabase`→`supabaseAdapter.signIn`, Android 검증) → 추후 별도 통합. local 모드에선 로그인 스킵.

> **기반(Foundation):** 이 브랜치는 `main`에서 분기한 상태 유지. spike의 Supabase/Capacitor/로그인 구현은 이번 브랜치에 합치지 않고 나중에 별도 통합. 따라서 단독 개발/실행은 **`VITE_HEARTPIN_API_MODE=local`** 기준(현재 `.env.local`은 `supabase` — main 기반에선 stub이라 로드 실패). 로그인 화면은 컴포넌트로 만들어 흐름에 끼워두되 실제 인증 게이트는 Supabase 통합 시 활성화.
...
```
src/mobile/
  MobileApp.jsx          (수정) 진입 게이트 라우팅: 스플래시 → (supabase&미로그인 시)로그인 → 첫실행 pick-side → 탭셸
  onboarding/
    LoginScreen.jsx      (신규) Supabase 로그인 UI (공유계정 email/password). auth seam 통과
    PickSide.jsx         (신규) 첫 실행 1회 바라/뇽이 선택 → settings.myChar 저장
  useAuth.js             (신규) 얇은 인증 seam: 세션 여부 + signIn/signOut (지금은 임시, Phase 1에서 Supabase 연결)
...
**진입 게이트 순서 (스플래시 우선):**
```
앱 열기 → LaunchSplash(자동, ~1.5s, 세션 체크 겸용)
        → [supabase 모드 & 세션 없으면 LoginScreen]
        → [첫 실행 / myChar 없으면 PickSide(1회)]
        → 여정
```
local 모드: 로그인 스킵(스플래시 → (첫실행 시)pick-side → 여정). 세션 있는 재방문자: 스플래시 → 여정. `MobileApp`이 `useAuth`(세션)·`api.getApiMode()`·`settings.myChar`로 게이트 판정.

- **진입 = 스플래시**(`LaunchSplash`). 버튼 없음. ~1.5s 후 다음 게이트로, 탭하면 즉시. (저감 모션 접근성: 탭 스킵 항상 가능)
